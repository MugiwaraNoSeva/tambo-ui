...
- [x] Parte 4 — e4099af — `puedeCargarEn()` es la única cuenta y su resultado viaja en el contexto del establecimiento activo; el de lectura ve el tambo entero y ninguna puerta de carga (alta, evento, tanque, anulación), y el que llega a mano por la barra de direcciones encuentra `SoloLectura` en vez de un formulario condenado al 403. Probado con los tres roles, incluido el admin con `permisos: []`
- [x] Parte 5 — 7d4c383 — `usuario` fuera de `CuerpoAlta` y `CuerpoEvento` (ninguna pantalla lo llenaba; el de la respuesta del log se queda), `/auth` y `/usuarios` en el proxy de desarrollo, y el README al día con cómo se entra, las credenciales de la demo y las cinco decisiones de la cerradura
